...
module RuleVariant = struct
 type t = Rule.t
 let compare rl rl' = if Rule.variant rl rl' then 0 else compare rl rl'
end

module RuleMap = Map.Make (RuleVariant)

module EqVariant = struct
 type t = Rule.t
 let compare (s,t) rl' = compare (s,t) rl'
(* 
  if Rule.variant (s,t) rl' || Rule.variant (t,s) rl' then 0 
  else compare (s,t) rl'*)
end

module EqMap = Map.Make (EqVariant)
...
let ht_iredtrss : (int, Rules.t) Hashtbl.t = Hashtbl.create 40;;
...
let rule_vars : (*(Rule.t * (Yices.expr * Yices.expr)) list ref = ref [];;*)
 Yices.expr RuleMap.t ref = ref RuleMap.empty
(* Yices variable for each rule *)
let eq_vars : (Rule.t * Yices.expr) list ref = ref [];;
 (*(Yices.expr * Yices.expr) EqMap.t ref = ref EqMap.empty*)
...
let rec store_eq_vars ctx rls =
 let add lr =
   if not (List.exists (fun (st,_) -> eq_variant st lr) !eq_vars) (*(EqMap.mem lr !eq_vars)*) then (
    ...
    wc := !wc + 1;
    eq_vars :=  (lr, (v,vi)) :: !eq_vars (*EqMap.add lr (v,vi) !eq_vars*)
    )
 in List.iter add rls
;;
