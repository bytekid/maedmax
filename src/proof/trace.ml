...
let mk_lit ts is_eq = { S.terms = ts; S.is_equality = is_eq; dconstr = [] }

let ancestors_with_subst eqs = 
  let mk eq = mk_lit eq true in
  let rec ancestors (eq0, sigma) =
    Format.printf "looking for ancestors of %a (substituted: %a)\n%!"
      Rule.print eq0 Rule.print (Rule.substitute sigma eq0);
    let eq = Variant.normalize_rule eq0 in
    let o = fst (trace_eq eq) in
    if o = Initial then 
      ( let sigma = Subst.compose (fst (rename_to eq eq0)) sigma in
        Format.printf " ... found initial %a (substituted: %a)\n%!"
      Rule.print eq Rule.print (Rule.substitute sigma eq);
        [mk eq, sigma])
    else
